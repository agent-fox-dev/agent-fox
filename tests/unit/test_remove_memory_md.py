...
class TestConstantRemoved:
    ...
    def test_constant_removed(self) -> None:
        ...
        source = (REPO_ROOT / "agent_fox" / "workspace" / "init_project.py").read_text()
...
class TestTemplateClean:
    ...
    def test_template_clean(self) -> None:
        ...
        content = (
            REPO_ROOT / "agent_fox" / "_templates" / "agents_md.md"
        ).read_text()
...
class TestProfileClean:
    ...
    def test_profile_clean(self) -> None:
        ...
        content = (
            REPO_ROOT / "agent_fox" / "_templates" / "profiles" / "agent.md"
        ).read_text()
        assert "memory.md" not in content
...
class TestSkillClean:
    ...
    def test_skill_template_clean(self) -> None:
        ...
        template = (
            REPO_ROOT / "agent_fox" / "_templates" / "skills" / "af-fix"
        ).read_text()
...
class TestInitTestsRemoved:
    ...
    def test_init_tests_removed(self) -> None:
        ...
        source = (
            REPO_ROOT / "tests" / "integration" / "test_init.py"
        ).read_text()
