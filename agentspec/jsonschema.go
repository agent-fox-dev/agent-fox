...
func ToolSchema(raw []byte) (*schema.Schema, error) {
	...
	if err != nil {
		return nil, fmt.Errorf("agentspec: decoding schema: %w", err)
	}
	if v.Kind != jsonx.KindObject {
		return nil, fmt.Errorf("agentspec: schema root is not an object")
	}
	...
	if s == nil {
		return nil, fmt.Errorf("agentspec: schema converted to nothing")
	}
	return s, nil
}
