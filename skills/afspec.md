...
The `spec` CLI creates and manages specifications in the **v1.3 JSON format**:
...
The AI-powered commands (`new`, `refine`, `generate`) require an Anthropic API
key:
...
| Variable | Purpose |
|----------|---------|
| `AF_SPEC_MODEL` | Override the default Claude model (tier name or model ID) |
| `SPEC_DIR` | Override the default spec directory (`.specs`); `--spec-dir` flag takes precedence |
...
Also check `.specs/steering.md` for project-level directives so the
...
3. Parse the JSON output to get the spec directory name (e.g. `{"ok": true, "spec_dir": "136_my_feature", "state": "init"}`).
...
5. If the refinement cycle needs to restart from scratch, use
   `spec refine --force <spec_dir_name>` to reset the session state,
   discarding all assessments, answers, and generated artifacts.

6. Repeat until the assessment returns `quality: "ready"`, but **do not
...
left off. To regenerate from scratch (deleting existing artifacts), use
`spec generate --force <spec_dir_name>`.
...
`.specs/<spec_dir>/architecture.md`.
...
When SPEC is omitted, validates all specs in the spec directory. Use `--cross`
to also run cross-spec interface consistency checks:
...
Use `--json` to get a JSON envelope instead of raw markdown output.
...
3. **Move** the old spec folder into `.specs/archive/`:
...
All spec files live under `.specs/NN_specification_name/`:
