...
## Step 7: Validate with lint-specs

After generating all 5 spec files, run the linter to catch structural issues:

1. Run `agent-fox lint-specs` on the generated spec folder.
2. Review any errors or warnings in the output.
3. Fix all lint errors and warnings before presenting the spec to the user.
4. Re-run `agent-fox lint-specs` to confirm all issues are resolved.

This automated validation catches missing sections, malformed requirement IDs,
broken cross-references, and other structural problems. It does NOT validate
semantic quality — use the manual checks in the completeness checklist below
for those.
...
After generating all 5 spec files and running `agent-fox lint-specs`, verify
the following before presenting to the user:
...
- [ ] Every domain-specific term used in criteria appears in the Glossary (manual check)
- [ ] No more than 10 requirements per spec (manual check)
...
- [ ] `## Execution Paths` section present with one numbered call chain per user-visible feature, naming exact module+function at each step and showing return types where data flows between steps (manual check)

### Requirements (requirements.md — additional checks)
- [ ] Every function whose output is consumed by a caller has a return value contract in its requirement (not just an action statement) (manual check)
...
- [ ] `## Integration Smoke Tests` section with one `TS-NN-SMOKE-N` entry per execution path from design.md (manual check)
- [ ] Each smoke test names which components must NOT be mocked (the ones in the execution path) (manual check)

### Implementation Tasks (tasks.md)
- [ ] Task group 1 is "Write failing spec tests"
- [ ] Every `TS-NN-N` entry is referenced by at least one task (manual check)
