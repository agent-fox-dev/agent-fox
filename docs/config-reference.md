# af Configuration Reference

This document lists every configuration option supported by agent-fox.
The simplified `config.toml` generated by `af init` contains only
...
Custom per-model pricing for cost tracking. By default af ships with
...
*For the simplified quick-start config, run `af init`.*
