...
af is an autonomous coding-agent orchestrator that drives AI coding agents through software development tasks without requiring continuous human supervision. A user describes what to build through specification documents; af takes it from there — planning work, dispatching agents, managing failures, merging results, and maintaining a persistent knowledge store — returning a finished feature branch and activity report when complete.

The product eliminates the primary pain point of AI-assisted development: the need to babysit an agent session by session, re-providing context, managing merge conflicts, and tracking what was done. af keeps working while the user is away.

---

## 2. Goals

- **Unattended execution:** A user should be able to initiate a multi-session coding run and walk away. af handles all coordination, retries, and merging autonomously.
...
- af does not write or modify the user's specifications — that remains a human or skill-assisted activity.
- af does not replace version control or project management tooling — it integrates with git and GitHub but does not own those systems.
- af is not a general-purpose AI chat interface; it has no conversational mode.
- af does not support AI providers other than Claude.
...
A pipeline or automation script that drives af in `--json` mode, consuming structured JSON output from every command, piping parameters via stdin, and reacting to exit codes. This persona has no interactive terminal.
...
1. The user installs af and navigates to their project root.
2. The user runs `af init --skills` to initialize the project.
3. af creates the project directory structure, a default configuration file, updates the ignore list, scaffolds an agent instructions document, creates the integration branch, and installs bundled skills as slash commands.
...
2. The user runs `af plan` to build an execution plan from the specifications.
3. af scans all specification folders, resolves task dependencies, and saves the plan.
4. The user runs `af code` (optionally with `--parallel N`) to start autonomous execution.
5. af dispatches agents to each ready task in dependency order. Reviewer agents run before and after coder agents. Coders work in isolated branches.
6. After each session, af harvests the result, merges clean commits to the integration branch, updates its knowledge store, and dispatches the next ready tasks.
7. The user runs `af status` at any time to see progress: task counts, token usage, cost, and any blocked or failed tasks.
...
1. The user runs `af fix` when quality checks are failing.
2. af detects available quality tools (test runner, linter, type checker), runs them, clusters failures by root cause, and generates fix specifications.
3. Coding agents resolve each cluster. af iterates until all checks pass or the maximum pass count is reached.
...
1. The user runs `af status` to see a dashboard: task counts by state (done, in-progress, pending, failed, blocked), total token usage, estimated cost, cost breakdown by agent role and by specification, and details on any blocked or failed tasks.
2. The user runs `af standup` to generate a report covering agent sessions, human commits, file overlaps between agent and human work, and the current task queue.
3. Both commands support JSON output for machine consumption.

### 4.6 Recovery and Retry

1. When tasks fail or become blocked, the user runs `af plan --reset` to clear failed and blocked tasks and retry them.
2. For targeted recovery, the user passes a specific task identifier: `af plan --reset TASK_ID`; af resets that task and unblocks its dependents.
3. For a full restart, the user runs `af plan --reset-hard`, which resets all tasks (including completed ones), cleans up branches and isolated working directories, compacts the knowledge store, and rolls back the integration branch to its pre-task state.
...
- WHEN a user runs `af init`, the system SHALL create the project directory structure, a configuration file, a steering document placeholder, an agent instructions document, the integration branch, and update the project's ignore list.
- WHEN `--skills` is provided, the system SHALL install all bundled skills as slash commands and report how many were installed.
- WHEN `af init` is run on a project that already has a configuration file, the system SHALL merge the existing file non-destructively: preserving active user values, adding new fields as commented-out entries, marking unrecognized active fields as deprecated, and making no changes if the file is already current.
- WHEN `af init` is run outside a git repository, the system SHALL exit with an error.

### 5.2 Planning

- WHEN a user runs `af plan`, the system SHALL scan the specifications directory, parse all task groups, build a dependency graph, resolve a topological task ordering, and save the plan.
...
- WHEN a plan already exists, `af plan` SHALL rebuild it from scratch on every invocation.

### 5.3 Autonomous Execution

- WHEN a user runs `af code`, the system SHALL dispatch coding sessions for all ready tasks in the plan, honoring dependency order.
...
- WHEN a user runs `af status`, the system SHALL display: task counts by state, total token usage, estimated cost, cost breakdown by agent role and by specification, and details on any problematic tasks.
- WHEN `--model` is provided, the system SHALL additionally display the execution model and critical-path analysis.
- WHEN a user runs `af standup`, the system SHALL report agent session activity, human commits, file overlaps between agent and human work, and queue status for the configured time window.
- WHEN `--json` is active, every command SHALL emit structured JSON to stdout and send all log messages to stderr only.

### 5.6 Quality Fix

- WHEN a user runs `af fix`, the system SHALL detect available quality check tools, run them, cluster failures by root cause, generate fix specifications, and dispatch coding sessions to resolve each cluster.
...
- WHEN a user runs `af plan --reset` without a task ID, the system SHALL prompt for confirmation and then reset all failed, blocked, and in-progress tasks, cleaning up their working directories and branches.
- WHEN a task identifier is provided (`af plan --reset TASK_ID`), the system SHALL reset only that task and unblock its dependents, without prompting.
...
- WHEN a user runs `af export --memory`, the system SHALL write all active knowledge facts grouped by category to stdout (or `docs/knowledge.json` in JSON mode).
- WHEN a user runs `af export --db`, the system SHALL write a full knowledge store dump to `.agent-fox/knowledge_dump.md` (or `.agent-fox/knowledge_dump.json` in JSON mode).
...
The project configuration lives in `.agent-fox/config.toml`. It is generated automatically by `af init`, which produces a fully-commented file with every available option, its description, valid range, and default value.
...
When the user re-runs `af init` after a tool upgrade, new configuration options are added as commented-out entries and removed options are marked deprecated, without disturbing any user-set values.

### 6.2 Specification Format

Specifications live under `.specs/NN_name/` (numbered by creation order) and contain five files: a PRD, acceptance criteria, design, test contracts, and a task list. The `af plan` command reads these files to build the execution graph.
...
- **`af init`:** Confirmation messages for each created or updated artifact, plus the count of skills installed when `--skills` is used.
- **`af plan`:** Summary of the built plan (task count, dependency edges, optional parallelism analysis).
- **`af code`:** Live session progress as tasks start, complete, or fail. Final summary on exit.
- **`af status`:** Dashboard showing task counts, token usage, cost totals, cost breakdowns, and problem task details.
- **`af standup`:** A human-readable daily activity report covering agent sessions, human commits, file overlaps, and queue status.
- **`af fix`:** Per-iteration pass/fail results and a final summary.
- **`af plan --reset`:** Confirmation of which tasks were reset and which working directories were cleaned up.
- **`af export`:** The destination path of the written file.
...
- Knowledge extracted from sessions is stored in a persistent knowledge store and accessible via `af export`.

### 7.4 Reports

- `af standup --output PATH` writes the standup report to the specified file.
- `af export --memory` writes knowledge facts to stdout (or JSON mode).
- `af export --db` writes `.agent-fox/knowledge_dump.md` (or `.agent-fox/knowledge_dump.json`).
---
...
|-----------|----------------------|
| Task session fails after all retries | Task marked blocked; reason shown in `af status` |
...
| Plan file missing for `code` or `status` | Error message directing the user to run `af plan` first; exits with code 1 |
...
- **Claude only:** af works exclusively with Claude models via the Anthropic SDK. No other AI provider is supported.
...
- **Specifications required before planning:** `af plan` and `af code` require well-formed specification files in `.specs/`. The product does not create specifications; that is a human or skill-assisted step.
