...
- Spec 120 (knowledge retrieval fixes): Fixed four read-side bugs in the knowledge retrieval pipeline. (1) `FoxKnowledgeProvider.set_run_id()` wires the run ID from the engine so that `_query_same_spec_summaries()` and `_query_cross_spec_summaries()` return results instead of empty lists. (2) Pre-review (group 0) findings are elevated into primary `[REVIEW]` results for non-zero task groups (tracked via `finding_injections`), and excluded from `[CROSS-GROUP]` to avoid duplication. (3) `generate_archetype_summary()` produces summaries for reviewer and verifier sessions; `query_same_spec_summaries()` no longer filters to `archetype='coder'` only. (4) `query_prior_run_findings()` and `query_prior_run_verdicts()` surface unresolved critical/major findings from prior runs as `[PRIOR-RUN]` items without tracking them in `finding_injections`. All changes are query-level; no schema changes. _(spec: 120_knowledge_retrieval_fixes, confidence: 0.95)_
- `review_findings.id` is a UUID column; test helpers that insert findings must use proper UUIDs (e.g. `str(uuid.uuid4())`), not arbitrary strings like `"F-001"` which cause `duckdb.ConversionException`. _(spec: 120_knowledge_retrieval_fixes, confidence: 0.95)_
- The `fallback_model` option (`[routing] fallback_model` and deprecated `[models] fallback_model`) was removed entirely in issue #594: the feature was not adding value. `resolve_fallback_model()`, `ResolvedSessionParams.fallback_model`, and the `fallback-model` extra_arg in ClaudeBackend are all gone. _(issue: 594, confidence: 1.0)_
