...
- **Reviewer** — a single archetype with three modes that cover all review
  roles:
  - *pre-flight* — reviews spec quality and validates spec assumptions
    against the actual codebase before implementation. Checks completeness,
    consistency, feasibility, security, and spec-to-code drift. Can block
    coding if critical findings exceed a threshold.
...
The knowledge system tracks five categories of context: review findings
(active critical and major findings for the current task group), drift
findings (spec-to-code discrepancies for the current spec), cross-group
findings (issues found in other groups of the same spec), cross-spec drift
(drift findings from other specs referencing overlapping files), and
same-spec session summaries (what earlier groups accomplished). Findings
follow a
