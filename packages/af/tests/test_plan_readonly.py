...
class TestPlanVerifyReadOnly:
    ...
    def test_verify_path_uses_read_only_true(
        ...
        with (
            ...
            result = cli_runner.invoke(
                main, ["plan", "--verify", "--specs-dir", str(specs_dir)]
            )
        ...
        if mock_oks.called:
            ...
            assert read_only_val is True, (
                f"af plan --verify must use read_only=True, got {read_only_val}"
            )
...
class TestPlanSaveReadWrite:
    ...
    def test_save_path_uses_read_only_false(
        self, cli_runner: CliRunner, tmp_path: Path
    ) -> None:
        ...
        with (
            ...
            result = cli_runner.invoke(
                main, ["plan", "--specs-dir", str(specs_dir)]
            )
        ...
        if mock_oks.called:
            ...
            for call in mock_oks.call_args_list:
                ...
                if read_only_val is False:
                    ...
                    break
            assert found_write_call, (
                "af plan save path must call open_knowledge_store with read_only=False"
            )
...
class TestDuckDBReadOnlyException:
    ...
    def test_read_only_connection_rejects_insert(self, tmp_path: Path) -> None:
        ...
        with pytest.raises(duckdb.InvalidInputException):
            conn_ro.execute(
                "INSERT INTO plan_nodes VALUES ('test', 'spec', 1, 'title')"
            )
        conn_ro.close()
