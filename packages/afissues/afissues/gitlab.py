...
def _map_issue(data: dict) -> IssueResult:
    return IssueResult(number=data["iid"], title=data["title"], html_url=data["web_url"], body=data.get("description") or "", labels=tuple(data.get("labels") or []))
...
class GitLabPlatform:
    ...
    async def _request(self, method: str, url: str, **kwargs: object) -> httpx.Response:
        return await request_with_retry(method, url, timeout=_GITLAB_TIMEOUT, transport=SSRFGuardTransport(), max_retries=_MAX_RETRIES, backoff_base=_RETRY_BACKOFF, **kwargs)
    ...
    async def create_issue(self, title: str, body: str, labels: list[str] | None = None) -> IssueResult:
        ...
        if resp.status_code != 201:
            raise IntegrationError(f"GitLab issue creation failed ({resp.status_code}): {_truncate_response(resp.text)}")
    ...
    async def list_issues_by_label(self, label: str, state: str = "open", *, sort: str = "created", direction: str = "asc") -> list[IssueResult]:
        params = {"labels": label, "state": _STATE_MAP.get(state, state), "order_by": _ORDER_BY_MAP.get(sort, sort), "sort": direction, "per_page": 100}
    ...
    async def add_issue_comment(self, issue_number: int, body: str) -> None:
        resp = await self._request("post", self._project_url(f"/issues/{issue_number}/notes"), json={"body": body}, headers=self._headers)
        if resp.status_code != 201:
            raise IntegrationError(f"GitLab comment creation failed ({resp.status_code}): {_truncate_response(resp.text)}")
    async def assign_label(self, issue_number: int, label: str) -> None:
        resp = await self._request("put", self._project_url(f"/issues/{issue_number}"), json={"add_labels": label}, headers=self._headers)
        if resp.status_code != 200:
            raise IntegrationError(f"GitLab label assignment failed ({resp.status_code}): {_truncate_response(resp.text)}")
    async def close_issue(self, issue_number: int, comment: str | None = None) -> None:
        if comment:
            await self.add_issue_comment(issue_number, comment)
        resp = await self._request("put", self._project_url(f"/issues/{issue_number}"), json={"state_event": "close"}, headers=self._headers)
        if resp.status_code != 200:
            raise IntegrationError(f"GitLab issue close failed ({resp.status_code}): {_truncate_response(resp.text)}")
    async def remove_label(self, issue_number: int, label: str) -> None:
        resp = await self._request("put", self._project_url(f"/issues/{issue_number}"), json={"remove_labels": label}, headers=self._headers)
        if resp.status_code != 200:
    ...
    async def list_issue_comments(self, issue_number: int) -> list[IssueComment]:
        ...
        resp = await self._request("get", self._project_url(f"/issues/{issue_number}/notes"), params=params, headers=self._headers)
        if resp.status_code != 200:
            raise IntegrationError(f"GitLab issue comments failed ({resp.status_code}): {_truncate_response(resp.text)}")
        return [IssueComment(id=note["id"], body=note["body"], user=note["author"]["username"], created_at=note["created_at"]) for note in resp.json() if not note.get("system", False)]
    async def get_issue(self, issue_number: int) -> IssueResult:
        resp = await self._request("get", self._project_url(f"/issues/{issue_number}"), headers=self._headers)
        if resp.status_code != 200:
    ...
    async def update_issue(self, issue_number: int, body: str) -> None:
        resp = await self._request("put", self._project_url(f"/issues/{issue_number}"), json={"description": body}, headers=self._headers)
        if resp.status_code != 200:
    ...
    async def create_label(self, name: str, color: str, description: str = "") -> None:
        ...
        resp = await self._request("post", self._project_url("/labels"), json=payload, headers=self._headers)
        if resp.status_code == 201:
            return None
    ...
    async def create_pr(self, *, title: str, body: str, head: str, base: str) -> str:
        ...
        resp = await self._request("post", self._project_url("/merge_requests"), json=payload, headers=self._headers)
        if resp.status_code == 201:
            return resp.json()["web_url"]
        if resp.status_code == 409:
            params = {"source_branch": head, "target_branch": base, "state": "opened"}
            fallback = await self._request("get", self._project_url("/merge_requests"), params=params, headers=self._headers)
            if fallback.status_code != 200:
                raise IntegrationError(f"GitLab MR creation returned 409 and fallback GET failed ({fallback.status_code}): {_truncate_response(fallback.text)}")
            mrs = fallback.json()
            if not mrs:
                raise IntegrationError(f"GitLab MR creation returned 409 (duplicate) but no existing open MR found for source={head!r} target={base!r}")
            return mrs[0]["web_url"]
        raise IntegrationError(f"GitLab MR creation failed ({resp.status_code}): {_truncate_response(resp.text)}")
    async def search_issues(self, query: str, state: str = "open") -> list[IssueResult]:
        params = {"search": query, "state": _STATE_MAP.get(state, state), "per_page": 100}
        resp = await self._request("get", self._project_url("/issues"), params=params, headers=self._headers)
        if resp.status_code != 200:
            raise IntegrationError(f"GitLab issue search failed ({resp.status_code}): {_truncate_response(resp.text)}")
        return [_map_issue(item) for item in resp.json()]
    async def check_credentials(self) -> None:
        resp = await self._request("get", f"{self._base_url}/projects/{self._encoded_project_id}", headers=self._headers)
        if resp.status_code in (401, 403):
            raise IntegrationError(f"GitLab credential check failed ({resp.status_code}): check that GITLAB_TOKEN is set to a valid token with api scope")
