...
Provides URL and transport-level validation to prevent Server-Side Request
Forgery (SSRF) attacks by rejecting requests targeting private, loopback,
link-local, reserved, multicast, and unspecified IP addresses.

Extracted from ``agentfox.platform.github`` (spec 04) so that both
``GitHubPlatform`` and ``GitLabPlatform`` share the same security layer.

Not re-exported from ``agentfox.platform.__init__``; the leading underscore
marks this module as internal.

Requirements: 04-REQ-18.1, 04-REQ-18.2, 04-REQ-18.3
"""
...
def _validate_transport_address(host: str) -> None:
    ...
    the actual TCP connection.  Unlike ``_validate_url``, a DNS failure
