...
from agentfox.fix.checks import CheckDescriptor, run_checks
...
class TestCollectorCapturesFailures:
    ...
    def test_failing_check_produces_failure_record(
        ...
        with patch("agentfox.fix.checks.subprocess.run", return_value=mock_result):
...
class TestCollectorTimeout:
    ...
    def test_timeout_recorded_as_failure(
        ...
        with patch(
            "agentfox.fix.checks.subprocess.run",
