...
from agentfox.fix.checks import CheckCategory, CheckDescriptor, run_checks
from agentfox.fix.fix import FixResult, TerminationReason, run_fix_loop
...
class TestFixLoopProgressEvents:
    ...
    async def test_fix_loop_emits_pass_start_event(self, tmp_path: Path) -> None:
        ...
        with (
            patch("agentfox.fix.fix.detect_checks", return_value=[mock_check]),
            patch(
                "agentfox.fix.fix.run_checks",
                return_value=([], [mock_check]),
            ),
        ):
    ...
    async def test_fix_loop_emits_clusters_found_event(self, tmp_path: Path) -> None:
        ...
        from agentfox.fix.checks import FailureRecord
        ...
        with (
            ...
            patch(
                "agentfox.fix.fix.cluster_failures",
                return_value=[cluster],
            ),
            patch("agentfox.fix.fix.generate_fix_spec", return_value=mock_spec),
            patch("agentfox.fix.fix.cleanup_fix_specs"),
        ):
            await run_fix_loop(
                tmp_path,
                config,
...
class TestImproveLoopProgressEvents:
    ...
    async def test_improve_loop_emits_analyzer_start_event(self, tmp_path: Path) -> None:
        ...
        with (
            ...
            patch("agentfox.fix.improve.parse_analyzer_response") as mock_parse,
        ):
            from agentfox.fix.analyzer import AnalyzerResult
...
class TestCheckCallbackEvents:
    ...
    def test_check_callback_called_on_start(self, tmp_path: Path) -> None:
        ...
        with patch("agentfox.fix.checks.subprocess.run") as mock_run:
