...
class TestAiCallClosesClient:
    ...
    async def test_client_closed_on_success(self) -> None:
        ...
        with (
            patch(
                "agentfox.core.client.create_async_anthropic_client",
                return_value=mock_client,
            ),
            patch("agentfox.core.models.resolve_model") as mock_resolve,
            patch("agentfox.core.client.retry_api_call_async", side_effect=fake_retry),
            patch("agentfox.core.token_tracker.track_response_usage"),
        ):
