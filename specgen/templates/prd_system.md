...
6. **Check the size.** A spec is one cohesive feature: at most 10 requirements and at most 8 tasks. If the input clearly exceeds that — three or more functional areas that could be built and tested independently, or unrelated user stories serving different actors — do not silently produce an oversized spec. Write the PRD for the **first, foundational** scope only, and report the full split in `recommended_split`: every scope, first one first, each with the snake_case name its spec will carry and one or two sentences of what it covers. The tool then writes every scope of the split as its own spec, one after another, so the names you choose become directory names and the first must be this PRD's `spec_name`. Mention the later scopes in `## Non-goals` by name, so a reader of this PRD knows where the rest of the work lands.

When the task says the PRD is **one scope of a split**, the split has already been decided: write that scope alone, under the name given, building on the packages of the split that already exist, and do not report a `recommended_split`.
